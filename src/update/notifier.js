...
module.exports = async client => {
	...
	const json = await (await fetch('https://api.github.com/repos/FluxNetworks/flux-tickets/releases')).json();
	const update = json[0];
	...
	if (semver.lt(current, latest)) {
		client.log.notice(`There is an update available for Flux Tickets (${current} -> ${update.tag_name})`);

		const lines = [
			`&k&6You are currently using &c${current}&6, the latest is &a${update.tag_name}&6.&r`,
			`&k&6Download "&f${update.name}&6" from&r`,
			link('&k&6the GitHub releases page.&r&6', 'https://github.com/FluxNetworks/flux-tickets/releases/')
		];
