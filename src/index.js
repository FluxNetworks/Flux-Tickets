/**
 * Flux Tickets
 * Copyright (C) 2021 IncasedDevelopment
 *
 ...
 * @description An open-source Flux bot for ticket management
...
process.title = 'Flux Tickets';

const min_node_version = '16.6.0';
const semver = require('semver');
if (semver.lt(process.versions.node, min_node_version)) return console.log(`\x07Error: Flux Tickets does not work on Node v${process.versions.node}; please upgrade to v${min_node_version} or above.`);
...
process.on('unhandledRejection', error => {
	...
	log.notice(`Flux Tickets v${version}, Node v${process.versions.node} on ${process.platform}`);
